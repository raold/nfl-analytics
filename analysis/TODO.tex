...
\begin{itemize}
  ...
  \item \prio{P1} Uncertainty-aware policy: downweight bets under wide posterior intervals. \done\ (uncertainty\_segmentation.py: 3-tier confidence segmentation)
\end{itemize>

\milestone{Profit Optimization (NEW - Expert Advisors Feedback)}
\block{EV-Optimal Selection}
\begin{itemize}
  \item \prio{P0} \textbf{Edge-based bet selection}: Replace probability thresholds with edge thresholds (3-4\% minimum). \done\ (ev\_bet\_selector.py: min\_edge filtering, CLV percentile tracking, no-bet deadzone)
  \item \prio{P0} \textbf{CLV tracking}: Filter bets by CLV percentiles that convert to profit. \done\ (integrated in ev\_bet\_selector.py with historical CLV analysis)
  \item \prio{P1} \textbf{No-bet deadzone}: Avoid marginal bets near breakeven (±20 bps). \done\ (ev\_bet\_selector.py: configurable deadzone parameter)
\end{itemize}

\block{Market Timing \& Dynamics}
\begin{itemize}
  \item \prio{P0} \textbf{Line movement velocity}: Calculate dLine/dt with weighted recency. \done\ (line\_movement\_enhanced.py: exponential weighting, half-life=6hrs)
  \item \prio{P0} \textbf{Market microstructure}: Cross-book consensus, hold calculation, arbitrage detection. \done\ (line\_movement\_enhanced.py: consensus calculator, hold metrics)
  \item \prio{P0} \textbf{Sharp vs public}: Detect reverse line moves, steam, professional alignment. \done\ (line\_movement\_enhanced.py: sharp\_public\_split detection)
  \item \prio{P1} \textbf{Timing optimization}: Wait vs bet-now decisions based on predicted movement. \done\ (line\_movement\_enhanced.py: \_determine\_bet\_timing method)
\end{itemize}
