...
\begin{tabular}{lrrr}
\toprule
CLV Percentile & Bet Count & Win Rate & ROI \\
\midrule
0-25\% & 2,847 & 48.2\% & -8.3\% \\
...
\begin{tabular}{lccc}
\toprule
Parameter & High Conf. & Medium Conf. & Low Conf. \\
\midrule
...
\begin{tikzpicture}[node distance=2cm, auto]
  % Nodes
  \node[rectangle, draw, text width=3cm, align=center] (models) {Prediction Models};
  \node[rectangle, draw, text width=3cm, align=center, right=of models] (edge) {Edge Calculator};
  \node[rectangle, draw, text width=3cm, align=center, right=of edge] (market) {Market Analysis};
  \node[rectangle, draw, text width=3cm, align=center, below=of edge] (uncertainty) {Uncertainty Segmentation};
  \node[rectangle, draw, text width=3cm, align=center, below=of market] (kelly) {Kelly + CVaR};
  \node[rectangle, draw, text width=3cm, align=center, below=of uncertainty] (router) {Book Router};
  \node[rectangle, draw, text width=3cm, align=center, below=of kelly] (execution) {Execution};

  % Arrows
  \draw[->] (models) -- (edge);
  \draw[->] (edge) -- (market);
  \draw[->] (market) -- (kelly);
  \draw[->] (edge) -- (uncertainty);
  \draw[->] (uncertainty) -- (kelly);
  \draw[->] (kelly) -- (router);
  \draw[->] (router) -- (execution);
\end{tikzpicture}
