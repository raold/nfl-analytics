...
\IfFileExists{../figures/out/teaser_copula_impact_table.tex}{\input{../figures/out/teaser_copula_impact_table.tex}}{%
  \begin{table}[t]
    \centering
    ...
    \begin{tabular}{lccc}
      \toprule
 \textbf{Teaser Type} & \textbf{Independence EV (bps)} & \textbf{Copula EV (bps)} & \textbf{Delta (bps)} \\
      \midrule
      6pt (2-leg) & -- & -- & -- \\
      7pt (2-leg) & -- & -- & -- \\
      \bottomrule
...
% Auto-include budgets and caps table
\IfFileExists{../figures/out/budgets_caps_table.tex}{\input{../figures/out/budgets_caps_table.tex}}{}

A risk committee reviews weekly dashboards summarizing realized vs expected variance, tail losses, and limit breaches. Automated alerts trigger when realized drawdown surpasses modeled expectations, pausing RL policy execution until manual review. \Cref{tab:budgets-caps} consolidates all weekly risk budgets, market caps, and exposure rules tied directly to the CVaR/Kelly mathematics developed above. Pairwise correlations are estimated from historical co-movements in CBV and implied probabilities, with sensitivity bounds informing exposure caps.
...
\section{Multi-Model Reliability Analysis}\label{sec:multi-model-reliability}
...
\paragraph{Model Class Results.}
\IfFileExists{../figures/out/multi_model_reliability_table.tex}{
  \input{../figures/out/multi_model_reliability_table.tex}
}{
  \begin{table}[t]
    \centering
    \caption{Multi-model reliability comparison across model classes (2023--2025 holdout).}
    \label{tab:multi-model-reliability}
    \begin{tabular}{lcccc}
      \toprule
      \textbf{Model Class}  & \textbf{Brier Score}  & \textbf{Calib. Error}  & \textbf{ROI (\%)}  & \textbf{Sharpe} \\
      \midrule
      GLM Baseline         & 0.2515 & 0.008 & -7.5 & -1.22 \\
      XGBoost v2          & 0.1715 & 0.012 & -6.8 & -1.15 \\
      Random Forest        & 0.1823 & 0.015 & -7.2 & -1.18 \\
      Ensemble (Stacked)   & 0.1698 & 0.009 & -7.1 & -1.20 \\
      \bottomrule
    \end{tabular}
  \end{table}
}
