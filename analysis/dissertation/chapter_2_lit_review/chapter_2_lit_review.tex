...
\]
estimated empirically by high/low quantile counts. Gaussian copulas imply $\lambda_U=\lambda_L=0$; $t$‑copulas yield $\lambda_U=\lambda_L>0$ depending on $\nu$ and $\rho$. We report $(\hat\lambda_U,\hat\lambda_L)$ with block bootstrap intervals to decide whether heavy‑tailed dependence is required (see \Cref{sec:unified-baseline-comparison} for empirical results).
...
\end{itemize}
We use Comparative Book Value (CBV, \Cref{subsec:vig-cbv-lit}) to identify mispricing: large positive CBV signals opportunities where our model assigns higher fair probability than the no-vig market price.
