...
\chaptersummary{
This dissertation demonstrated that NFL betting markets are semi-strong form efficient: sophisticated models achieve positive closing-line value (+14.9 bps CLV) and strong calibration (Brier 0.2515), yet fail to overcome the vigorish barrier (ROI = $-7.5$\%, Sharpe = $-1.22$). The system's value lies not in profitability but in its methodological contributions: a complete system architecture integrating calibrated prediction, offline RL, copula-based dependence modeling, and transparent failure analysis. These methods transfer to alternative domains where market efficiency is lower and friction costs are smaller. The thesis validates that uncertainty quantification + governance converts edge into reliable predictions, even when economic profits remain elusive.
}{
Future work should focus on extending the framework to less efficient markets (player props, live betting, international leagues) and portfolio optimization across multiple sports, while maintaining the core principles of uncertainty quantification and governance that underpin sustainable risk management.
}
