...
cat("BAYESIAN RECEIVING MODEL WITH QB-WR CHEMISTRY\n")
cat(paste0(rep("=", 80), collapse=""), "\n\n")
...
# ============================================================================
cat("Step 2: Loading receiving data with QB information...\n")

query <- "
WITH passing_plays AS (
  -- Get QB for each play
  SELECT DISTINCT
    p.game_id,
    p.play_id,
...
SELECT
  pgs.player_id as receiver_id,
  pgs.season,
  pgs.week,
  pgs.stat_yards as yards,
  pgs.stat_receptions as receptions,
  pgs.stat_targets as targets,

  -- QB info (from plays table)
  pp.qb_id,
  pp.qb_name,
...
LEFT JOIN LATERAL (
  SELECT DISTINCT ON (p.game_id, p.receiver_player_id)
    p.passer_player_id as qb_id,
    p.passer_player_name as qb_name
  FROM plays p
  WHERE p.game_id = g.game_id
    AND p.receiver_player_id = pgs.player_id
    AND p.passer_player_id IS NOT NULL
  ORDER BY p.game_id, p.receiver_player_id, p.play_id DESC
) pp ON TRUE

WHERE pgs.stat_category = 'receiving'
  AND pgs.stat_yards IS NOT NULL
  AND pgs.stat_targets IS NOT NULL
  AND pgs.stat_targets >= 3  -- Minimum targets
  AND pgs.season BETWEEN 2020 AND 2024
  AND ph.position_group IN ('WR', 'TE', 'RB')
  AND pp.qb_id IS NOT NULL  -- Must have QB identified
...
cat("Generating posterior predictions (this may take a few minutes)...\ n")
