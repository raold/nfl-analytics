...
def compute_key_mass_delta(
    hist_masses: dict[int, float], sim_masses: dict[int, float]
) -> dict[int, float]:
    """
...
def compute_dependence_delta(hist_dep: dict, sim_dep: dict) -> float:
    """
...
def run_acceptance_tests(
    ...
    margin_emd = compute_emd(hist["margin_distribution"]["pmf"], sim["margin_distribution"]["pmf"])
    print(f"✅ Margin EMD: {margin_emd:.4f}")

    # Test 2: Total EMD
    print("\n3. Computing total distribution EMD...")
    total_emd = compute_emd(hist["total_distribution"]["pmf"], sim["total_distribution"]["pmf"])
    print(f"✅ Total EMD: {total_emd:.4f}")

    # Test 3: Key number masses
    print("\n4. Computing key number mass deltas...")
    key_deltas = compute_key_mass_delta(hist["key_number_masses"], sim["key_number_masses"])
    print("✅ Key mass deltas:")
    for key, delta in key_deltas.items():
        print(
            f"   {int(key):2d} points: {delta*100:.2f}% "
    ...
    dep_delta = compute_dependence_delta(hist["score_dependence"], sim["score_dependence"])
