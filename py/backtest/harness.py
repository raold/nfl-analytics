...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--calibrations", default=None, help="Comma-separated calibration methods to sweep (overrides --calibration)")
    ap.add_argument("--cv-folds", type=int, default=5)
    ap.add_argument("--thresholds", default="0.45,0.50,0.55", help="Comma-separated decision thresholds to sweep")
    ap.add_argument("--cal-bins", type=int, default=10, help="Bins for reliability curve")
    ap.add_argument("--cal-out-dir", default=None, help="Directory to write reliability plots/CSVs per run and per season")
    ap.add_argument("--panel-config", default=None, help="Config name to build per-season reliability panel for")
    ap.add_argument("--panel-cal", default=None, help="Calibration to build per-season reliability panel for")
    ap.add_argument("--panel-threshold", type=float, default=None, help="Threshold to build per-season reliability panel for")
    ap.add_argument("--panel-tex", default=None, help="Output TeX path for per-season reliability panel")
    ap.add_argument("--panel2-cal", default=None, help="Second calibration for side-by-side panel (e.g., none)")
    ap.add_argument("--panel-combo-tex", default=None, help="Output TeX path for side-by-side (cal1 vs cal2) panel")
...
def write_tex(df: pd.DataFrame, path: str) -> None:
    ...
    cols = ["config", "calibration", "season", "games", "ece", "brier", "log_loss", "hit_rate", "roi"]
    lines = [
...
def write_tex_overall(df: pd.DataFrame, path: str) -> None:
    os.makedirs(os.path.dirname(path), exist_ok=True)
    cols = ["config", "calibration", "threshold", "ece", "mce", "brier", "log_loss", "hit_rate", "roi"]
    lines = [
        "% !TEX root = ../../main/main.tex",
        "\\begin{table}[t]",
        "  \\centering",
...
def write_dual_panel_tex(left_images: list[str], right_images: list[str], out_path: str, left_title: str, right_title: str) -> None:
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
...
def main() -> None:
    ...
    calibrations = [m.strip() for m in (args.calibrations.split(',') if args.calibrations else [args.calibration]) if m.strip()]
        ...
        for cal in calibrations:
            ...
            for thr in thresholds:
                ...
                if not preds_df.empty:
                    ...
                    ece = float((rel["count"] * (rel["pred_mean"] - rel["obs_rate"]).abs()).sum() / total)
                    ...
                    for s, g in preds_df.groupby("season"):
                        ...
                        season_ece[int(s)] = float((rel_s["count"] * (rel_s["pred_mean"] - rel_s["obs_rate"]).abs()).sum() / tot_s)
                    ...
                    metrics_df["ece"] = metrics_df["season"].map(lambda x: season_ece.get(int(x), np.nan))
                    metrics_df = pd.concat([metrics_df, overall_df.drop(columns=["config", "calibration", "threshold"])], ignore_index=True)
                    ...
                    if args.cal_out_dir:
                        ...
                        try:
                            ...
                            pass
                        try:
                            glm.write_reliability_plot(rel, os.path.join(args.cal_out_dir, base + "_overall.png"))
                        except Exception:
                            pass
                        ...
                        for s, g in preds_df.groupby("season"):
                            ...
                            try:
                                ...
                                pass
                            try:
                                glm.write_reliability_plot(rel_s, os.path.join(args.cal_out_dir, f"{base}_s{s}.png"))
                            except Exception:
                                pass
                        ...
                        if (
                            ...
                            and (
                                args.panel_threshold is None or abs(args.panel_threshold - thr) < 1e-9
                            )
                        ):
                            ...
                            for s in seasons_sorted:
                                ...
                                rel_path = os.path.relpath(rel_img, start=os.path.join(ROOT, "analysis", "dissertation", "figures", "out"))
    ...
    if args.panel_combo_tex and combo_left and combo_right and combo_left_title and combo_right_title:
        write_dual_panel_tex(combo_left, combo_right, args.panel_combo_tex, combo_left_title, combo_right_title)
