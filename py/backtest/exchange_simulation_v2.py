...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.02,
        help='Minimum EV to place bet (0.02 = 2%% edge)'
    )
    parser.add_argument(
        '--output',
        type=Path,
    ...
    if sim['roi'] > 0:
        ...
        print(f"     This proves the model can identify +EV opportunities!")
    else:
        print(f"[X] Not yet profitable. ROI = {sim['roi']:.2f}%")
        print(f"    Average edge: {sim['avg_edge']:.2%}")
        print(f"    Model may need better calibration or different features")
