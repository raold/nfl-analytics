...
class BootstrapStressTest:
                ...
                if delta_wins > 0:
                    ...
                    if len(win_indices) >= abs(delta_wins):
                        flip_indices = self.rng.choice(
                            win_indices, size=abs(delta_wins), replace=False
                        )
    ...
    def stress_test_scenarios(
        self, outcomes: np.ndarray, bet_sizes: np.ndarray, n_trials: int = 1000
    ...
    for bet in bets:
        ...
        if "bet_won" in bet:
            ...
            if isinstance(won, bool):
                ...
                outcome = 1 if won == 1.0 or won else 0
        elif "won" in bet:
...
def main():
    ...
    parser.add_argument("--results", type=str, required=True, help="Path to strategy results JSON")
    parser.add_argument(
        "--output",
        type=str,
    ...
    parser.add_argument("--n-trials", type=int, default=1000, help="Number of bootstrap trials")
    ...
    output_data = {scenario_name: asdict(metrics) for scenario_name, metrics in results.items()}
    ...
    print("\n" + "=" * 120)
    print(
        f"{'Scenario':<30} {'Mean Ret':<10} {'Sharpe':<10} {'Sortino':<10} {'Max DD':<10} {'VaR(95%)':<12} {'CVaR(95%)':<12}"
    )
    print("=" * 120)
