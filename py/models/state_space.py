...
class StateSpaceRatings:
    ...
    def get_ratings_df(self) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            self.history,
            columns=["season", "week", "team", "rating", "variance"]
        )
...
def evaluate_model(model: StateSpaceRatings, df: pd.DataFrame) -> Dict[str, float]:
    ...
    metrics = {
        ...
        "accuracy_win": float(np.mean([
            (p >= 0.5) == bool(a) for p, a in zip(probs_win, actuals_win)
        ]))
    }
    ...
    if probs_ats:
        ...
        metrics["accuracy_ats"] = float(np.mean([
            (p >= 0.5) == bool(a) for p, a in zip(probs_ats, actuals_ats)
        ]))
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--q", type=float, default=3.0, help="Process noise std (weekly strength volatility)")
    ...
    ap.add_argument("--init-variance", type=float, default=100.0, help="Initial team rating variance")
