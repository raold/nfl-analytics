...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default='models/bayesian',
    ...
    bnn = RushingBNN_PriorSensitivity(
        ...
        activation='relu',
        prior_std=0.5,
        noise_sigma=args.sigma  # KEY PARAMETER
    )
    ...
    # Map test players
    player_idx_test = df_test['player_id'].map(bnn.player_encoder).fillna(len(bnn.player_encoder) - 1).astype(int).values
    ...
    if in_90 > 0.85 and in_90 < 0.95:
        ...
        logger.info(f"  ✗ Still under-calibrated")
    ...
    results = {
        ...
        'features': 'baseline_4',
        'calibration': {
            '90%_coverage': float(in_90),
            '68%_coverage': float(in_68)
        },
        'accuracy': {
            'mae': float(mae),
            'rmse': float(rmse)
