...
class RushingBNN:
    ...
    def load_data(
        ...
        df = pd.read_sql(
            query,
            conn,
            params=[start_season, end_season, start_season, end_season,
    ...
    def prepare_features(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        feature_cols = [
            # Player features
            'rush_attempts', 'avg_rushing_yards_l3', 'avg_ypc_l3',
            'height', 'weight', 'age',
            'is_rb', 'is_qb', 'is_fb',

            # Team/game features
            'team_rush_epa', 'team_rush_success_rate',
            'def_rush_epa_allowed', 'def_rush_success_allowed',
            'oline_quality',

            # Interaction features
            'attempts_x_ypc', 'oline_x_def',
    ...
    def predict(
        ...
        with self.model:
            ...
            posterior_predictive = pm.sample_posterior_predictive(
                self.trace,
                var_names=['prediction', 'uncertainty'],
                progressbar=False
            )
...
def main():
    ...
    top_rbs = test_df.groupby('player_name').agg({
        'rushing_yards': 'mean',
        'prediction': 'mean',
        'uncertainty': 'mean'
    ...
    for player, row in top_rbs.iterrows():
        logger.info(f"  {player:20s}: Actual={row['rushing_yards']:.1f}, "
