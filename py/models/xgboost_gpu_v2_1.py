#!/usr/bin/env python3
"""
XGBoost GPU-accelerated trainer for NFL win probability prediction (v2.1 MODEL).

v2.1 improvements over v2:
- Multi-season validation (2022, 2023, 2024, 2025) instead of single test season
- Focused hyperparameter sweep around best v2 config (depth=3, lr=0.05, n_est=300)
...
Usage:
    ...
    python py/models/xgboost_gpu_v2_1.py \
        ...
        --end-season 2021 \
        --test-seasons 2022 2023 2024 2025 \
        --output models/xgboost/v2_1/best_model.json \
        --params models/xgboost/v2_1_sweep/best_config.json
"""
...
DEFAULT_FEATURES = [
    ...
    # Season context
    "week",
    # v2 features: 4th down coaching (ablation study showed these drive 97% of improvement)
    "fourth_downs_diff",
...
def focused_hyperparameter_sweep(
    ...
    for i, params in enumerate(param_grid):
        ...
        try:
            ...
            print(
                f"  Mean Test Brier: {result['mean_test_brier']:.4f} "
            ...
            for season_result in result["season_results"]:
                print(
                    f"    Season {season_result['test_season']}: "
...
def main():
    ...
    parser.add_argument(
        "--start-season", type=int, default=2006, help="First season to include in training"
    )
    parser.add_argument(
        "--end-season",
        type=int,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("models/xgboost/v2_1_sweep"),
        help="Output directory for sweep results",
    )
    parser.add_argument(
        "--params", type=Path, help="JSON file with hyperparameters for single model training"
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    if args.sweep:
        ...
        print(f"\n[DONE] v2.1 sweep complete. Results saved to {args.output_dir}")
        ...
        print(
            f"  Mean Test Brier: {result['mean_test_brier']:.4f} (±{result['std_test_brier']:.4f})"
        )
        print(f"  Mean Test AUC: {result['mean_test_auc']:.4f}")
        print("\nPer-Season Performance:")
        for season_result in result["season_results"]:
            print(
                f"  {season_result['test_season']}: "
                f"Brier {season_result['test_brier']:.4f}, "
                f"Acc {season_result['test_accuracy']:.3f}, "
