...
def main():
    ...
    if metrics.coverage_90 < 0.85:
        print(
            f"✗ SEVERE CALIBRATION ISSUE: 90% CI only covers {metrics.coverage_90:.1%} of samples"
        )
        print(f"  → Intervals are {0.90/metrics.coverage_90:.1f}x too narrow")
        print("  → Need to widen uncertainty estimates")

    if metrics.coverage_68 < 0.63:
        print(
            f"✗ SEVERE CALIBRATION ISSUE: 68% CI only covers {metrics.coverage_68:.1%} of samples"
        )
    ...
    print("  → Intervals are SHARP but POORLY CALIBRATED")
