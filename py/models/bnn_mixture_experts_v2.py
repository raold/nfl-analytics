...
class MixtureExpertsBNN:
    ...
    def build_model(self, X_train, y_train):
        ...
        with pm.Model() as model:
            ...
            W_gate = pm.Normal('W_gate', mu=0, sigma=0.5,
                              shape=(D, K))
            ...
            for k in range(K):
                ...
                W_h = pm.Normal(f'W_expert{k}_h', mu=0, sigma=1,
                               shape=(D, H))
                b_h = pm.Normal(f'b_expert{k}_b', mu=0, sigma=1,
                               shape=H)
                h = pm.math.tanh(X @ W_h + b_h)

                W_out = pm.Normal(f'W_expert{k}_out', mu=0, sigma=1,
                                 shape=(H, 1))
            ...
            sigma_experts = []
            for k in range(K):
                sigma_k = pm.HalfNormal(f'sigma_expert{k}',
                                        sigma=10 + 5*k)  # 10, 15, 20
    ...
    def train(self, X_train, y_train, n_samples=2000, n_tune=1000, n_chains=4):
        ...
        """
        print(f"Building Mixture-of-Experts BNN")
...
def main():
    ...
    print("="*80)
    print(f"                    Phase 1    Phase 2.2   Improvement")
