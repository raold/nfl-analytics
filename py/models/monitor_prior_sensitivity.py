    ...
    if completed:
        ...
        if df is not None:
            ...
            for _, row in df.iterrows():
                ...
                print(
                    f"{row['sigma']:<8.1f} {coverage_90:<12.1%} {row['coverage_68']:<12.1%} "
            ...
            if len(completed) == 4:
                ...
                if analysis:
                    ...
                    print(
                        f"   90% Coverage: {analysis['optimal_coverage_90']:.1%} (target: 85-95%)"
                    )
                    print(
                        f"   68% Coverage: {analysis['optimal_coverage_68']:.1%} (target: 60-75%)"
                    )
                    ...
                    if analysis["is_well_calibrated"]:
                        ...
                        print(
                            f"   Recommendation: Use sigma={analysis['optimal_sigma']:.1f} for production"
                        )
                    else:
                        ...
                        if analysis["optimal_coverage_90"] < 0.85:
                            print(
                                f"   Coverage still low - consider testing sigma > {max(completed):.1f}"
                            )
                        elif analysis["optimal_coverage_90"] > 0.95:
                            print(
                                "   Coverage too high - consider interpolating between tested values"
                            )
