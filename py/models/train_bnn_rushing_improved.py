...
class ImprovedRushingBNN:
    ...
    def build_model(self, X: np.ndarray, y: np.ndarray, player_idx: np.ndarray):
        ...
        self.model = model
        logger.info(
            f"Built hierarchical BNN with {self.hidden_dim} hidden units and {n_players} player effects"
        )
    ...
    def train(
        ...
        if divergences > total_samples * 0.01:
            ...
            logger.info("✓ Divergence rate acceptable")

        if float(rhat_prediction["prediction"].max()) < 1.01:
            logger.info("✓ R-hat indicates good convergence")
        else:
            logger.warning("⚠️  R-hat > 1.01 indicates convergence issues")
    ...
    def predict(
        ...
        if player_idx_test.max() >= n_training_players:
            logger.warning(
                f"⚠️  Found player index {player_idx_test.max()} >= {n_training_players}, clipping to valid range"
            )
...
def main():
    ...
    bnn = ImprovedRushingBNN(
        hidden_dim=16, activation="relu", prior_std=0.5  # Simpler architecture  # Tighter priors
    )
    ...
    if n_unseen > 0:
        logger.info(
            f"⚠️  {n_unseen} test samples from unseen players - mapping to average player effect"
        )
    ...
    if in_68 > 0.60 and in_68 < 0.75:
        logger.info("  ✓ ±1σ calibration is GOOD")
