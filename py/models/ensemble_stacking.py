...
class EnsembleStacker:
    ...
    def __init__(self, v2_model_path: str, v3_model_path: str, features_csv: str):
        ...
        with open(self.v2_model_path.parent / 'metadata.json', 'r') as f:
        ...
        with open(self.v3_model_path.parent / 'metadata.json', 'r') as f:
    ...
    def train_stacking_models(
        ...
        logger.info(f"  Train Brier: {results['fixed']['train_brier']:.4f}, Acc: {results['fixed']['train_acc']:.1%}")
        logger.info(f"  Val Brier: {results['fixed']['val_brier']:.4f}, Acc: {results['fixed']['val_acc']:.1%}")
        ...
        logger.info(f"  Train Brier: {results['logistic']['train_brier']:.4f}, Acc: {results['logistic']['train_acc']:.1%}")
        logger.info(f"  Val Brier: {results['logistic']['val_brier']:.4f}, Acc: {results['logistic']['val_acc']:.1%}")
        ...
        logger.info(f"  Train Brier: {results['optimal']['train_brier']:.4f}, Acc: {results['optimal']['train_acc']:.1%}")
        logger.info(f"  Val Brier: {results['optimal']['val_brier']:.4f}, Acc: {results['optimal']['val_acc']:.1%}")
        ...
        logger.info(f"  Train Brier: {results['xgboost_meta']['train_brier']:.4f}, Acc: {results['xgboost_meta']['train_acc']:.1%}")
        logger.info(f"  Val Brier: {results['xgboost_meta']['val_brier']:.4f}, Acc: {results['xgboost_meta']['val_acc']:.1%}")
    ...
    def evaluate_on_test(
        ...
        logger.info(f"  Optimal ({weights['v3']:.2f}/{weights['v2']:.2f}): Brier {optimal_brier:.4f}, Accuracy {optimal_acc:.1%}")
    ...
    def save_ensemble(self, stacking_results: Dict, test_results: Dict, output_dir: str):
        ...
        import joblib
        joblib.dump(stacking_results['logistic']['model'],
                   output_path / 'logistic_stacker.pkl')

        # Save XGBoost meta-learner
        stacking_results['xgboost_meta']['model'].save_model(
            str(output_path / 'xgboost_meta.json'))
        ...
        metadata = {
            ...
            'stacking_approaches': {
                ...
                'logistic': {
                    k: v for k, v in stacking_results['logistic'].items() if k != 'model'
                }
            },
