#!/usr/bin/env python3
"""
BNN with Opponent Defense Features - Phase 1 Extension

Adds opponent defensive features to Vegas features to test if opponent strength
improves calibration beyond the 29.7% achieved with Vegas alone.

Features:
- Baseline (4): carries, avg_rushing_l3, season_avg, week
- Vegas (2): spread_close, total_close
- Opponent Defense (3): opp_rush_yds_allowed_avg, opp_rush_rank, opp_rush_yds_l3
= Total: 9 features

Hypothesis: Opponent defensive strength affects rushing performance variance,
...
class RushingBNN_Opponent:
    """BNN with Vegas + Opponent Defense features for improved calibration"""

    def __init__(self, hidden_dim: int = 16, activation: str = "relu", prior_std: float = 0.5):
        """Initialize BNN with Opponent Defense features."""
        ...
        logger.info(f"Initialized BNN with Opponent Defense features ({hidden_dim} hidden units)")

    def load_data(self, start_season: int = 2020, end_season: int = 2024) -> pd.DataFrame:
        """Load rushing data WITH Vegas lines + opponent defense features"""
        ...
        opponent_defense AS (
            ...
            SELECT
                ...
                -- Season rushing yards allowed average up to this week
                AVG(pgs.stat_yards) OVER (
                    PARTITION BY pgs.current_team, pgs.season
                    ORDER BY pgs.week
                    ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING
                ) as rush_yds_allowed_avg,
                -- Recent 3-game rushing yards allowed
                AVG(pgs.stat_yards) OVER (
        ...
        opponent_ranks AS (
            -- Rank defenses by rushing yards allowed
            SELECT
                season,
                week,
                defense_team,
                rush_yds_allowed_avg,
                rush_yds_allowed_l3,
                ROW_NUMBER() OVER (
            ...
            FROM (
                SELECT DISTINCT season, week, defense_team, rush_yds_allowed_avg, rush_yds_allowed_l3
        ...
        logger.info(
            f"✓ Opponent defense: {df['opp_rush_yds_allowed_avg'].notna().sum()} games with stats"
        )
        logger.info(
            f"  Avg opponent rush yds allowed: {df['opp_rush_yds_allowed_avg'].mean():.1f} yards"
        )
        logger.info(
            f"  Opponent rank range: [{df['opp_rush_rank'].min():.0f}, {df['opp_rush_rank'].max():.0f}]"
        )

        return df

    def prepare_features(self, df: pd.DataFrame) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Prepare feature matrix with Vegas + opponent defense features"""
        ...
        # Feature set: baseline (4) + Vegas (2) + opponent defense (3)
        feature_cols = [
            # Baseline features
            "carries",
            "avg_rushing_l3",
        ...
        logger.info(
            f"✓ Feature matrix shape: {X.shape} (9 features: 4 baseline + 2 Vegas + 3 opponent defense)"
        )
        logger.info(f"  Avg opponent rush yds allowed: {X[:, 6].mean():.1f} yards")
        logger.info(f"  Avg opponent rank: {X[:, 7].mean():.1f}")
        logger.info(f"  Avg opponent rush yds L3: {X[:, 8].mean():.1f} yards")
...
def main():
    """Train and evaluate BNN with Opponent Defense features"""

    logger.info("=" * 80)
    logger.info("BNN WITH OPPONENT DEFENSE FEATURES - Phase 1 Extension")
    logger.info("Testing if opponent defensive strength improves calibration")
    logger.info("=" * 80)

    # Initialize model
    bnn = RushingBNN_Opponent(hidden_dim=16, activation="relu", prior_std=0.5)

    # Load data with opponent defense features
    logger.info("\nLoading rushing data with opponent defense features...")
    ...
    # Train model
    logger.info("\n" + "=" * 80)
    logger.info("TRAINING BNN WITH OPPONENT DEFENSE FEATURES")
    ...
    logger.info("\nCalibration (WITH OPPONENT DEFENSE FEATURES):")
    ...
    if in_90 > 0.85 and in_90 < 0.95:
        logger.info("  ✓ 90% CI calibration is GOOD - Opponent defense features worked!")
    elif in_90 > 0.297:
        ...
        logger.info("  ✗ No improvement from opponent defense features")

    # Save model
    model_path = "models/bayesian/bnn_rushing_opponent.pkl"
    bnn.save_model(model_path)

    # Save results
    results = {
        "experiment_name": "opponent_defense_bnn_9features",
        "config": {
            "model": "BNN Hierarchical + Vegas + Opponent Defense",
        ...
        "notes": "BNN with opponent defense features: baseline (4) + Vegas (2) + opponent defense (3).\n"
        "Testing if opponent defensive strength improves uncertainty quantification.",
    }

    results_path = "experiments/calibration/opponent_defense_bnn_9features.json"
    ...
    logger.info("\n" + "=" * 80)
    logger.info("✓ BNN WITH OPPONENT DEFENSE FEATURES TRAINING COMPLETE")
    logger.info("=" * 80)
