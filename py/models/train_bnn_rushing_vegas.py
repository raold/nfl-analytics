...
class RushingBNN_Vegas:
    ...
    def load_data(self, start_season: int = 2020, end_season: int = 2024) -> pd.DataFrame:
        ...
        logger.info(
            f"✓ Vegas lines: {df['spread_close'].notna().sum()} games with spread, "
...
def main():
    ...
    # Initialize model
    bnn = RushingBNN_Vegas(hidden_dim=16, activation="relu", prior_std=0.5)
    ...
    if in_90 > 0.85 and in_90 < 0.95:
        logger.info("  ✓ 90% CI calibration is GOOD - Vegas features helped!")
    elif in_90 > 0.26:
        logger.info("  ↗ Improvement over baseline, but still under-calibrated")
    else:
        logger.info("  ✗ No improvement from Vegas features")
