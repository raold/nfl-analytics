...
def main():
    ...
    if not model_path.exists():
        ...
        print(
            "  Training may still be in progress. Check: tail -f models/bayesian/bnn_rushing_vegas_training.log"
        )
        return
    ...
    print("✓ Model architecture rebuilt with 6 features")
    ...
    logger.log_experiment(
        experiment_name="vegas_bnn_6features", config=config, metrics=metrics, notes=notes.strip()
    )
    ...
    if baseline_path.exists():
        ...
        if metrics.coverage_90 >= 0.85 and metrics.coverage_90 <= 0.95:
            ...
            print(
                f"  Coverage improved from {baseline_metrics['coverage']['90pct']:.1%} to {metrics.coverage_90:.1%}"
            )
