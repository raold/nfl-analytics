...
class PerformanceDashboard:
    ...
    def create_dashboard(self):
        ...
        st.set_page_config(
            page_title="NFL Props Performance Dashboard", page_icon="🏈", layout="wide"
        )
        ...
        try:
            df = self.load_performance_data(
                start_date, end_date, None if model_version == "All" else model_version
            )
            ...
            with col1:
                st.metric(
                    "ROI", f"{metrics['roi']:.2f}%", f"{metrics['roi'] - 1.59:.2f}% vs baseline"
                )
            ...
            with col4:
                st.metric("Sharpe Ratio", f"{metrics['sharpe_ratio']:.2f}", "Risk-adjusted return")
            ...
            # Win rate by week
            weekly_stats = (
                df.groupby(["week", "model_version" if model_version == "All" else None])
                .agg({"win": "mean", "profit": "sum", "bet_amount": "sum"})
                .reset_index()
            )
            ...
            with col2:
                st.metric("Avg Odds", f"{metrics['avg_odds']:.2f}", "Average betting odds")

            with col3:
                st.metric("MAE", f"{metrics['mae']:.1f} yards", "Mean absolute error")
