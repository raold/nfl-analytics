...
class CalibrationTracker:
    ...
    def load_predictions_with_outcomes(
        ...
        if model_version:
            query = query.replace(
                "WHERE pr.updated_at", "AND pr.model_version = %s\nWHERE pr.updated_at"
            )
            params.insert(0, model_version)

        if prop_type:
            query = query.replace("WHERE actual", "AND prop_type = %s\nWHERE actual")
    ...
    def calculate_coverage(
        self, df: pd.DataFrame, confidence_levels: list[float] = [0.50, 0.68, 0.90, 0.95]
    ...
    def calculate_quantile_calibration(self, df: pd.DataFrame) -> pd.DataFrame:
    ...
    def create_reliability_diagram(self, df: pd.DataFrame, n_bins: int = 10) -> go.Figure:
    ...
    def create_calibration_plot(self, df: pd.DataFrame) -> go.Figure:
    ...
    def detect_calibration_drift(self, df: pd.DataFrame, window_size: int = 50) -> pd.DataFrame:
    ...
    def save_calibration_metrics(
        self, metrics: dict[float, CalibrationMetrics], model_version: str
    ):
    ...
    def generate_report(
        self, start_date: datetime, end_date: datetime, model_version: str | None = None
    ) -> dict:
        ...
        if df.empty:
            return {"status": "error", "message": "No predictions with outcomes found"}
        ...
        summary = {
            ...
            "mean_calibration_error": np.mean(
                [m.calibration_error for m in coverage_metrics.values()]
            ),
            "max_calibration_error": max([m.calibration_error for m in coverage_metrics.values()]),
            "calibration_status": (
                "GOOD"
                if max([m.calibration_error for m in coverage_metrics.values()]) < 0.05
        ...
        if summary["coverage_50"]:
            ...
            for level in [50, 68, 90, 95]:
                ...
                if summary[key]:
                    metric = summary[key]
                    logger.info(
                        f"  {level}% CI: {metric.observed_coverage:.1%} "
