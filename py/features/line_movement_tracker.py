...
class LineMovementTracker:
    ...
    def analyze_game(
        ...
        sharp_indicators = self.identify_sharp_indicators(game_id, opening_spread, closing_spread)
    ...
    def get_summary_stats(self) -> dict:
        ...
        return {
            ...
            "games_with_sharp_indicators": sum(
                1 for m in self.movements if len(m.sharp_indicators) > 0
            ),
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Line Movement Tracker for Early Week Betting (EWB)")
...
def main():
    ...
    print(f"{'='*80}")
    print("Line Movement Tracker - Early Week Betting (EWB) Analysis")
    print(f"{'='*80}")
    ...
    if args.game and args.opening_line is not None and args.closing_line is not None:
        ...
        if movement.steam_moves:
            ...
            for steam in movement.steam_moves:
                print(
                    f"  - {steam['timestamp']}: {steam['spread_movement']:+.1f} points in {steam['time_window_minutes']:.0f} min"
                )
    ...
    # Historical analysis
        ...
        print(
            f"  Games with sharp indicators: {stats['games_with_sharp_indicators']} ({stats['games_with_sharp_indicators']/stats['total_games']*100:.1f}%)"
        )
        print(
            f"  Games with steam moves: {stats['games_with_steam_moves']} ({stats['games_with_steam_moves']/stats['total_games']*100:.1f}%)"
        )
        ...
        if args.analyze_clv:
            ...
            print(
                f"  EWB wins (better line): {clv.ewb_wins} ({clv.ewb_wins/clv.total_bets*100:.1f}%)"
            )
            print(
                f"  EWB losses (worse line): {clv.ewb_losses} ({clv.ewb_losses/clv.total_bets*100:.1f}%)"
            )
            print(
                f"  Pushes (same line): {clv.ewb_pushes} ({clv.ewb_pushes/clv.total_bets*100:.1f}%)"
            )
