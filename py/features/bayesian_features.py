...
def merge_bayesian_features(
    ...
    if missing_home > 0 or missing_away > 0:
        print(f"⚠ Warning: {missing_home} home teams and {missing_away} away teams missing Bayesian ratings")
        ...
        games_df["bayesian_combined_sd"] = games_df["bayesian_combined_sd"].fillna(2.12)  # sqrt(2*1.5^2)
...
def add_bayesian_prediction(games_df: pd.DataFrame, home_advantage: float = 2.4) -> pd.DataFrame:
    ...
    """
    games_df["bayesian_pred_margin"] = (
        games_df["bayesian_rating_diff"] + home_advantage
    )

    # Convert margin to win probability (assume ~13.5 point SD from empirical data)
    margin_sd = 13.5
    from scipy.stats import norm
    games_df["bayesian_prob_home"] = norm.cdf(
        games_df["bayesian_pred_margin"] / margin_sd
    )
...
def print_feature_summary(games_df: pd.DataFrame):
    ...
    if "bayesian_rating_diff" in games_df.columns:
        ...
        print(f"  Range: [{games_df['bayesian_rating_diff'].min():.2f}, {games_df['bayesian_rating_diff'].max():.2f}]")
...
def main():
    ...
    print(f"  Input:  {len(games_df.columns) - len([c for c in games_df.columns if 'bayesian' in c])} features")
    print(f"  Output: {len(games_df.columns)} features (+{len([c for c in games_df.columns if 'bayesian' in c])} Bayesian)")
