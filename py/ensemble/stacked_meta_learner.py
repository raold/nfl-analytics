...
class StackedMetaLearner:
    ...
    def _engineer_meta_features(
        ...
        features = pd.DataFrame({
            ...
            'models_agree': (
                (bayesian_preds > 0.5) == (xgb_preds > 0.5)
            ).astype(int),
            ...
            'min_confidence': np.minimum(
                np.abs(bayesian_preds - 0.5),
                np.abs(xgb_preds - 0.5)
            ),
            'max_confidence': np.maximum(
                np.abs(bayesian_preds - 0.5),
                np.abs(xgb_preds - 0.5)
            ),
        })
        ...
        if self.use_uncertainty and bayesian_uncertainty is not None:
            ...
            features['uncertainty_adjusted_pred'] = (
                bayesian_preds / (bayesian_uncertainty + 1e-6)
            )
    ...
    def fit(
        ...
        X_xgb: np.ndarray,
        bayesian_uncertainty: Optional[np.ndarray],
        y: np.ndarray,
        context: Optional[pd.DataFrame] = None,
        cv_splits: int = 5
    ) -> Dict[str, float]:
        ...
        logger.info(f"✓ Meta-learner trained: CV = {metrics['cv_mean']:.4f} ± {metrics['cv_std']:.4f}")
        return metrics
    ...
    def _get_feature_names(self) -> List[str]:
        ...
        base_features = [
            'bayesian_pred', 'xgb_pred', 'pred_diff', 'pred_ratio',
            'models_agree', 'bayesian_confidence', 'xgb_confidence',
            'min_confidence', 'max_confidence'
        ]

        if self.use_uncertainty:
            base_features.extend([
                'bayesian_uncertainty', 'uncertainty_adjusted_pred'
            ])
...
class DynamicWeightEnsemble:
    ...
    def fit(
        ...
        optimal_weights = self._compute_optimal_weights(
            bayesian_preds, xgb_preds, y
        )
    ...
    def predict(
        ...
        bayesian_weights = np.clip(
            self.weight_model.predict(features_scaled), 0, 1
        )

        # Weighted ensemble
        ensemble_preds = (
            bayesian_weights * bayesian_preds +
            (1 - bayesian_weights) * xgb_preds
        )
...
if __name__ == "__main__":
    ...
    metrics = meta_learner.fit(
        bayesian_preds, xgb_preds, bayesian_uncertainty, y
    )

    # Get predictions
    stacked_preds = meta_learner.predict_proba(
        bayesian_preds, xgb_preds, bayesian_uncertainty
    )
