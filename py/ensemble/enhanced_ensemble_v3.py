    ...
    def get_4way_ensemble_predictions(
        ...
        if self.use_stacking and self.meta_learner.is_fitted:
            ...
            meta_features = pd.DataFrame({
                ...
                # Consensus
                'mean_pred': (merged['bayesian_pred'] + merged['xgb_pred'] + merged['bnn_pred']) / 3,
            ...
            merged['ensemble_pred'] = (
                (merged['bayesian_pred'] / bayesian_var +
                 ...
                 merged['bnn_pred'] / bnn_var) / total_precision
            )
        ...
        merged['ensemble_uncertainty'] = (
            merged['bayesian_uncertainty'] + merged['bnn_uncertainty']
        ) / 2 * 0.8  # Ensemble reduces uncertainty
...
def compare_enhancements():
    ...
    logger.info(f"Theoretical utilization: ~40% → ~85%")
    logger.info("="*60 + "\n")
