...
from ensemble_predictor import XGBoostPredictor, CQLPredictor, IQLPredictor
...
def backtest_thompson_sampling(
    ...
    # Prepare features
    xgb_feats = test_df[xgb_features].to_numpy()
    ...
    xgb_actions = np.array([
        xgb_model.get_action(xgb_probs[i], market_probs[i])
    ...
    model_selection_dist = {
        name: ts.selections[name] / n_bets if n_bets > 0 else 0.0
        for name in ts.model_names
    }
...
def main():
    ...
    print(f"{'='*80}")
    print(f"Thompson Sampling Meta-Policy (Task 7)")
    print(f"{'='*80}")
    ...
    for model, stats in results['thompson_sampling_stats'].items():
        ...
        print(f"    95% CI: [{stats['credible_interval'][0]:.3f}, {stats['credible_interval'][1]:.3f}]")
