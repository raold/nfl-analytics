...
class KickoffTimeFixer:
    ...
    def fetch_espn_schedule(self, season: int, week: int) -> list[dict]:
        ...
        params = {"week": week, "seasontype": 2, "dates": season}  # Regular season
            ...
            for event in data["events"]:
                try:
                    ...
                    if not home_team or not away_team:
                        logger.warning(
                            f"Could not determine home/away teams for event {event.get('id')}"
                        )
                        continue
    ...
    def parse_kickoff_time(self, game: dict) -> str | None:
        """
        ...
        try:
            ...
            logger.info(
                f"  {game['away_team']} @ {game['home_team']}: {kickoff_utc} -> {utc_timestamp}"
            )
