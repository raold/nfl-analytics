...
def load_predictions(predictions_path: Path) -> pd.DataFrame:
    ...
    cols_keep = [
        'game_id', 'season', 'week',
        'home_win_actual',
        'prob_glm', 'prob_xgb', 'prob_state', 'prob_ensemble'
    ]
...
def merge_predictions_features(
...
def main():
    ...
    if not args.include_all_features:
        ...
        cols_export = [
            ...
            'spread_close', 'total_close', 'home_cover',
            'prob_glm', 'prob_xgb', 'prob_state', 'prob_ensemble',
            'predicted_margin', 'ensemble_edge', 'ensemble_error', 'brier_contrib',
            # Key features for explainability
            'home_prior_epa_mean', 'away_prior_epa_mean', 'prior_epa_mean_diff',
            'home_epa_pp_last3', 'away_epa_pp_last3', 'epa_pp_last3_diff',
            'home_prior_margin_avg', 'away_prior_margin_avg', 'prior_margin_avg_diff',
            'home_rest_days', 'away_rest_days', 'rest_days_diff',
            'home_qb_change', 'away_qb_change', 'qb_change_diff',
            'home_prev_result', 'away_prev_result'
        ]
