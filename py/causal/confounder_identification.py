...
class ConfounderIdentifier:
    ...
    def identify_confounders(
        ...
        for var in candidate_confounders:
            ...
            treatment_assoc = self._test_treatment_association(
                df, var, treatment_col
            )

            # Test association with outcome
            outcome_assoc = self._test_outcome_association(
                df, var, outcome_col, treatment_col
            )

            # Variable is a confounder if associated with both
            is_confounder = (
                treatment_assoc['significant'] and
                outcome_assoc['significant']
            )
            ...
            confounders[var] = {
                ...
                'strength': self._compute_confounder_strength(
                    treatment_assoc, outcome_assoc
                )
            }
    ...
    def _test_treatment_association(
        self,
        df: pd.DataFrame,
        var: str,
        treatment_col: str
    ...
    def _compute_confounder_strength(
        self,
        treatment_assoc: Dict,
        outcome_assoc: Dict
    ) -> float:
        """
    ...
    def get_minimal_adjustment_set(
        ...
        adjustment_set = [
            var for var, info in confounders.items()
    ...
    def check_positivity(
        ...
        violation = (
            extreme_low > len(propensity_scores) * 0.05 or
            extreme_high > len(propensity_scores) * 0.05 or
            overlap_prop < min_overlap
        )
        ...
        return {
            ...
            'recommendation': 'Trim extreme propensity scores' if violation else 'Overlap sufficient'
        }
    ...
    def detect_colliders(
        ...
        for var in candidate_variables:
            ...
            treatment_assoc = self._test_treatment_association(
                clean_df, var, treatment_col
            )

            # Test if outcome predicts variable
            outcome_pred = self._test_treatment_association(
                clean_df, var, outcome_col
            )
...
def main():
    ...
    positivity = identifier.check_positivity(
        mock_df, 'injury', adjustment_set
    )
    ...
    colliders = identifier.detect_colliders(
        ...
        'injury',
        'yards',
        ['pro_bowl', 'player_ability', 'shoe_size']
    )
