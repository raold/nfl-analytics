...
class CausalModelIntegrator:
    ...
    def add_causal_features(self, df: pd.DataFrame, feature_set: str = "standard") -> pd.DataFrame:
        """
        ...
        logger.info(
            f"Added {feature_set} causal features: {df_causal.shape[1] - df.shape[1]} new columns"
        )
    ...
    def _add_deconfounded_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "stat_carries" in df.columns and "recent_avg_yards" in df.columns:
            ...
            if valid_idx.sum() > 50:
                lr = LinearRegression()
                lr.fit(df.loc[valid_idx, ["recent_avg_yards"]], df.loc[valid_idx, "stat_carries"])

                # Residual = actual - predicted (removes ability confounding)
                df_deconf["deconfounded_carries"] = np.nan
                df_deconf.loc[valid_idx, "deconfounded_carries"] = df.loc[
                    valid_idx, "stat_carries"
    ...
    def _add_propensity_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "injury_treatment" in df.columns:
            ...
            covariates = [
                col
                for col in ["stat_carries", "recent_avg_yards", "player_age", "games_played"]
            ...
            if len(covariates) > 0:
                ...
                if valid_idx.sum() > 100:
                    ...
                    lr.fit(df.loc[valid_idx, covariates], df.loc[valid_idx, "injury_treatment"])
                    ...
                    df_ps.loc[~treated, "ipw_weight"] = 1 / (
                        1 - df_ps.loc[~treated, "injury_propensity"]
                    )
    ...
    def estimate_treatment_effect_priors(
        self, treatment_type: str, outcome: str = "rushing_yards"
    ) -> dict[str, float]:
        """
        ...
        return (
            self.injury_effects.get(outcome)
            or self.coaching_change_effects.get(outcome)
            or self.trade_effects.get(outcome)
        )
    ...
    def adjust_prediction_for_shock(
        ...
        logger.info(
            f"Shock adjustment ({shock_type}): {base_prediction:.1f} → {adjusted_prediction:.1f} "
    ...
    def generate_counterfactual_prediction(
        self, player_id: str, week: int, season: int, intervention: dict[str, Any]
    ) -> dict:
        """
    ...
    def compute_betting_edge_with_causality(
        self, prediction: float, market_line: float, causal_adjustments: dict | None = None
    ) -> dict:
        """
        ...
        if causal_adjustments:
            ...
            for shock_type, magnitude in causal_adjustments.items():
                adjustment = self.adjust_prediction_for_shock(adjusted_pred, shock_type, magnitude)
            ...
            return {"prediction": prediction, "market_line": market_line, "edge": base_edge}

    def identify_high_leverage_events(self, upcoming_games: pd.DataFrame) -> pd.DataFrame:
        """
        ...
        if "post_coaching_change" in df_leverage.columns:
            ...
            df_leverage["causal_leverage_score"] += df_leverage["post_coaching_change"] * (
                5 - np.minimum(weeks_since, 5)
            )

        if "post_trade" in df_leverage.columns:
            weeks_since_trade = df_leverage.get("weeks_since_trade", 999)
            df_leverage["causal_leverage_score"] += df_leverage["post_trade"] * (
                4 - np.minimum(weeks_since_trade, 4)
            )
        ...
        logger.info(
            f"Identified {(df_leverage['causal_leverage_score'] > 2).sum()} high-leverage events"
        )
...
def main():
    ...
    adjusted = integrator.adjust_prediction_for_shock(
        base_prediction=base_pred, shock_type="injury", shock_magnitude=1.0
    )
    ...
    for _, row in leverage_games.iterrows():
        ...
        print(
            f"{row['player_name']:<15} {row['causal_leverage_score']:<15.1f} {', '.join(events):<30}"
        )
