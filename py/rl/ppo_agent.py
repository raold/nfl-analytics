...
def train_ppo_offline(
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            print(
                f"Epoch {epoch+1}/{n_epochs} | Reward: {avg_reward:.4f} | Action: {avg_action:.4f}"
            )
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="models/ppo_model.pth", help="Output path for model"
    )
    parser.add_argument("--load", type=str, help="Load model from checkpoint")
    parser.add_argument("--epochs", type=int, default=200, help="Number of training epochs")
    parser.add_argument(
        "--rollout-length", type=int, default=512, help="Rollout length before PPO update"
    )
    ...
    parser.add_argument(
        "--device", type=str, default="cpu", choices=["cpu", "cuda", "mps"], help="Device"
    )
