...
class LoggedFillsBacktest:
    ...
    def __init__(
        self,
        dataset_path: str,
        params: Optional[FillSimulationParams] = None,
        random_seed: int = 42
    ):
        """
        ...
        # Validate dataset
        required_cols = ['game_id', 'season', 'week', 'model_prob', 
                        'market_prob', 'outcome', 'clv']
    ...
    def evaluate_policy(
        ...
        for sim in range(n_simulations):
            results = self.simulate_fills(
                actions, bet_sizes, model_probs, market_probs, 
                outcomes, clvs
            )
            ...
            if len(filled_bets) > 0:
                ...
                total_wagered = sum(r.bet_size for r in filled_bets)
    ...
    def generate_utilization_table(
        self,
        comparison_df: pd.DataFrame,
        output_path: Path
    ):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Backtest RL policies with realistic fills"
    )
    parser.add_argument(
        "--dataset", type=str, default="data/rl_logged.csv",
    ...
    backtest.generate_comparison_table(
        comparison,
        args.output_dir / "rl_vs_baseline_table.tex"
    )
    
    backtest.generate_utilization_table(
        comparison,
        args.output_dir / "utilization_adjusted_sharpe_table.tex"
    )
