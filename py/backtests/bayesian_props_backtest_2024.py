...
class BayesianPropsBacktest:
    ...
    def run_weekly_backtest(
        ...
        for week in range(start_week, end_week + 1):
            ...
            actuals = self.fetch_actual_outcomes(
                stat_type=stat_type,
                season=season,
                weeks=[week]
            )
            ...
            # Calculate errors
            week_results['error'] = (
                week_results['bayes_prediction'] - week_results['actual_yards']
            )
    ...
    def calculate_metrics(self, results: pd.DataFrame) -> Dict:
        ...
        if not high_data.empty:
            metrics['mae_high_data'] = mean_absolute_error(
                high_data['actual_yards'],
                high_data['bayes_prediction']
            )

        if not low_data.empty:
            metrics['mae_low_data'] = mean_absolute_error(
                low_data['actual_yards'],
                low_data['bayes_prediction']
            )
            ...
            for pos in results['player_position'].unique():
                ...
                if len(pos_data) > 10:  # Minimum sample size
                    metrics[f'mae_{pos}'] = mean_absolute_error(
                        pos_data['actual_yards'],
                        pos_data['bayes_prediction']
                    )
        ...
        if 'bayes_reliability_score' in results.columns:
            reliability_correlation = results[['abs_error', 'bayes_reliability_score']].corr().iloc[0, 1]
            metrics['reliability_validity'] = -reliability_correlation  # Should be positive if valid
    ...
    def compare_with_baseline(
        ...
        if not valid_results.empty:
            comparison['bayes_mae'] = mean_absolute_error(
                valid_results['actual_yards'],
                valid_results['bayes_prediction']
            )
            comparison['baseline_mae'] = mean_absolute_error(
                valid_results['actual_yards'],
                valid_results['baseline_prediction']
            )
            comparison['mae_improvement'] = (
                (comparison['baseline_mae'] - comparison['bayes_mae']) /
                comparison['baseline_mae'] * 100
            )

            # Win rate (which model was closer)
            bayes_closer = (
                np.abs(valid_results['bayes_prediction'] - valid_results['actual_yards']) <
    ...
    def simulate_betting_performance(
        self,
        results: pd.DataFrame,
        props_lines: Optional[pd.DataFrame] = None,
        ...
        if props_lines is None:
            ...
            results['prop_line'] = results['bayes_prediction'] * np.random.uniform(0.95, 1.05, len(results))
        else:
            results = results.merge(
        ...
        for _, row in results.iterrows():
            ...
            actual_prob_over = stats.norm.cdf(
                row['prop_line'],
                loc=row['bayes_prediction'],
                scale=row['bayes_uncertainty']
            )
        ...
        return {
            ...
            'max_drawdown': (max(bankroll_history) - min(bankroll_history)) / max(bankroll_history) * 100
        }
    ...
    def generate_report(
        ...
        results: pd.DataFrame,
        metrics: Dict,
        output_dir: str = 'reports/bayesian_backtest'
    ):
        ...
        for level in coverage_levels:
            lower = results['bayes_prediction'] - stats.norm.ppf((1+level)/2) * results['bayes_uncertainty']
            upper = results['bayes_prediction'] + stats.norm.ppf((1+level)/2) * results['bayes_uncertainty']
            coverage = ((results['actual_yards'] >= lower) & (results['actual_yards'] <= upper)).mean()
        ...
        ax = axes[1, 1]
        bins = pd.qcut(results['bayes_data_strength'], q=5, labels=['Very Low', 'Low', 'Medium', 'High', 'Very High'])
        ...
        p = np.poly1d(z)
        ax.plot(results['bayes_reliability_score'].sort_values(),
               p(results['bayes_reliability_score'].sort_values()), "r-", alpha=0.8)
        ...
        ax.errorbar(x_pos, sample_players['bayes_prediction'],
                   yerr=sample_players['bayes_uncertainty'] * 1.96,
                   fmt='o', alpha=0.6, label='95% CI')
        ax.scatter(x_pos, sample_players['actual_yards'], color='red', marker='x', s=50, label='Actual')
        ax.set_xticks([])
    ...
    def run_full_backtest(
        ...
        for stat_type in stat_types:
            ...
            self.generate_report(
                results,
                metrics,
                output_dir=f'reports/bayesian_backtest/{stat_type}'
            )
