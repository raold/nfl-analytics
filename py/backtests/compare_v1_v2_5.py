...
def main():
    ...
    v1_metrics = calculate_metrics(
        comparison, "pred_yards_v1", "actual_yards", "pred_q05_v1", "pred_q95_v1"
    )

    v2_5_metrics = calculate_metrics(
        comparison, "pred_yards_v2_5", "actual_yards", "pred_q05_v2_5", "pred_q95_v2_5"
    )
    ...
    ax = axes[0, 1]
    ax.scatter(
        comparison["actual_yards"], comparison["pred_yards_v2_5"], alpha=0.6, s=100, color="green"
    )
    ax.plot([0, 600], [0, 600], "r--", lw=2, label="Perfect prediction")
    ax.set_xlabel("Actual Yards/Game", fontsize=12)
    ax.set_ylabel("Predicted Yards/Game", fontsize=12)
    ax.set_title(
        f'v2.5 Informative Priors\nMAE: {v2_5_metrics["mae"]:.1f}, r={v2_5_metrics["correlation"]:.3f}',
        fontsize=12,
        fontweight="bold",
    )
    ax.legend()
    ...
    ax.axhline(
        ...
        linestyle="--",
        alpha=0.7,
        label=f'v1.0 MAE: {v1_metrics["mae"]:.1f}',
    )
    ax.axhline(
        v2_5_metrics["mae"],
        color="orange",
        linestyle="--",
        alpha=0.7,
        label=f'v2.5 MAE: {v2_5_metrics["mae"]:.1f}',
    )
    ...
    corr_improvement = (
        (v2_5_metrics["correlation"] - v1_metrics["correlation"]) / v1_metrics["correlation"]
    ) * 100
