...
class ModelVersionComparison:
    ...
    def fetch_v1_predictions(self, stat_type='passing_yards'):
        ...
        return df[['player_id', 'pred_v1', 'uncertainty_v1', 'ci_lower_v1', 'ci_upper_v1', 'n_games_observed']]
    ...
    def load_bnn_predictions(self, actuals: pd.DataFrame):
        ...
        try:
            ...
            bnn = BayesianNeuralNetwork.load(bnn_path)
    ...
    def calculate_metrics(self, actuals: np.ndarray, predictions: np.ndarray, model_name: str) -> dict:
    ...
    def run_comparison(self, stat_type='passing_yards'):
        ...
        v1_metrics = self.calculate_metrics(
            comparison['actual_yards'].values,
            comparison['pred_v1'].values,
            'v1.0 Baseline'
        )

        # Calculate CI coverage for v1.0
        in_ci_90 = (
            (comparison['actual_yards'] >= comparison['ci_lower_v1']) &
    ...
    def generate_comparison_report(self, stat_type='passing_yards'):
        ...
        ax2.axvline(residuals_v1.mean(), color='blue', linestyle='--', linewidth=2, label=f'Mean: {residuals_v1.mean():.1f}')
        ...
        coverage_data = pd.DataFrame({
            'Model': ['v1.0'],
            'Coverage': [metrics['v1.0']['ci_90_coverage']],
            'Expected': [0.90]
        })
        x = np.arange(len(coverage_data))
        width = 0.35
        ax3.bar(x - width/2, coverage_data['Coverage'], width, label='Actual', color='steelblue')
        ax3.bar(x + width/2, coverage_data['Expected'], width, label='Expected', color='lightcoral')
        ...
        ax5.text(0.05, 0.95, f'r = {corr_unc_err:.3f}', transform=ax5.transAxes,
        ...
        weekly_errors = comparison.groupby('week').agg({
            'error_v1': 'mean',
            'actual_yards': 'count'
        }).reset_index()
        ax8.plot(weekly_errors['week'], weekly_errors['error_v1'], 'o-', color='steelblue', linewidth=2)
        ax8.set_xlabel('Week')
        ...
        plt.suptitle(f'{stat_type.replace("_", " ").title()} - Model Version Comparison (2024)',
