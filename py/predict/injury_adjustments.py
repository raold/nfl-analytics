...
class InjuryAdjuster:
    ...
    def get_depth_chart_position(
        self, gsis_id: str, team: str, season: int, week: int
    ) -> tuple[str, int]:
        """
    ...
    def calculate_team_impact(
        self, injuries: pd.DataFrame, team: str, season: int, week: int
    ) -> float:
        """
        ...
        for _, injury in team_injuries.iterrows():
            ...
            position, depth = self.get_depth_chart_position(injury["gsis_id"], team, season, week)
        ...
        if impacts_detail:
            ...
            for detail in impacts_detail:
                logger.info(
                    f"  {detail['player']} ({detail['position']}{detail['depth']}): {detail['impact']:+.3f}"
                )
            logger.info(f"  TOTAL: {total_impact:+.3f}")

        return total_impact

    def adjust_predictions(self, predictions: pd.DataFrame, season: int, week: int) -> pd.DataFrame:
        """
    ...
    def adjust_from_csv(self, predictions_csv: str, season: int, week: int, output_csv: str):
        """
...
def main():
    ...
    if args.predictions_csv:
        ...
        adjusted = adjuster.adjust_from_csv(args.predictions_csv, args.season, args.week, output)
        ...
        for team in sorted(injuries["team"].unique()):
            ...
            for _, inj in team_injuries.iterrows():
                print(
                    f"  - {inj['full_name']} ({inj['position']}) - {inj['report_primary_injury']}"
                )
