...
class AutoRetrainingPipeline:
    ...
    def detect_latest_season(self) -> Optional[int]:
        """
    ...
    def regenerate_features(self, through_season: int) -> bool:
        ...
        cmd = [
            ...
            '--output', str(self.project_root / f'data/processed/features/asof_team_features_v3_{through_season}.csv')
        ]
    ...
    def retrain_v3_model(self, features_csv: str, through_season: int, test_season: int) -> Dict:
        ...
        cmd = [
            ...
            '--start-season', '2006',
            '--end-season', str(through_season),
            '--test-seasons', str(test_season),
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.info(f"✓ v3 Model - Brier: {test_metrics.get('brier_score', 0):.4f}, "
    ...
    def retrain_spread_model(self, features_csv: str, through_season: int, test_season: int) -> Dict:
        """
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                test_metrics = metadata['test_results'].get(str(test_season), {})
                logger.info(f"✓ Spread Model - MAE: {test_metrics.get('mae', 0):.2f}, "
    ...
    def retrain_totals_model(self, features_csv: str, through_season: int, test_season: int) -> Dict:
        """
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.info(f"✓ Totals Model - MAE: {val_metrics.get('mae', 0):.2f}, "
    ...
    def compare_and_promote(
        ...
        try:
            with open(prod_v3_path, 'r') as f:
                prod_v3 = json.load(f)
            with open(prod_spread_path, 'r') as f:
                prod_spread = json.load(f)
            with open(prod_totals_path, 'r') as f:
        ...
        v3_improved = new_v3_metrics.get('brier_score', 1.0) < prod_v3['test_results'].get('2024', {}).get('brier_score', 1.0)
        spread_improved = new_spread_metrics.get('mae', 100) < prod_spread['test_results'].get('2024', {}).get('mae', 100)
        totals_improved = new_totals_metrics.get('mae', 100) < prod_totals['validation_metrics'].get('mae', 100)
    ...
    def _promote_models(self, through_season: int) -> bool:
        ...
        logger.info(f"✓ New models promoted to production")
    ...
    def run_pipeline(
        ...
        # Compare and promote
        promoted = self.compare_and_promote(v3_metrics, spread_metrics, totals_metrics, through_season)
...
def main():
    ...
    parser.add_argument('--auto', action='store_true',
                       help='Auto-detect latest completed season')
    parser.add_argument('--season', type=int,
                       help='Season to retrain on (required if not --auto)')
