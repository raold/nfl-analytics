...
def get_game_data(
    conn: psycopg.Connection, season_start: int, season_end: int
...
def fit_copula_parameters(margins: np.ndarray, totals: np.ndarray) -> dict[str, dict[str, float]]:
    ...
    return {"gaussian": {"rho": rho}, "t": {"rho": rho, "df": df}}


def probability_cover_spread(
    spread: float, teaser_points: float, margin_mean: float, margin_std: float
) -> float:
...
def teaser_ev_independent(p_spread: float, p_total: float, payout: float = 1.5) -> float:
...
def generate_teaser_ev_table(
    ...
    for scenario in scenarios:
        for points in teaser_points:
            ...
            p_spread = probability_cover_spread(scenario["spread"], points, margin_mean, margin_std)
            p_total = probability_cover_total(
                scenario["total"], points, total_mean, total_std, scenario["direction"]
            )
            ...
            ev_gaussian = teaser_ev_copula(
                p_spread, p_total, copula_params["gaussian"]["rho"], payout, "gaussian"
            )

            # EV under t-copula
            ev_t = teaser_ev_copula(
                p_spread, p_total, copula_params["t"]["rho"], payout, "t", copula_params["t"]["df"]
            )
...
def generate_copula_impact_summary(results: list[dict], output_path: Path):
    ...
    lines = [
        ...
        r"    \multicolumn{3}{l}{overestimates EV by $\sim$"
        + f"{abs(mean_delta_g)*100:.1f}"
        + r"\% on average.} \\",
        r"    \bottomrule",
        r"  \end{tabular}",
...
def main():
    parser = argparse.ArgumentParser(description="Generate teaser pricing comparison tables")
    parser.add_argument("--season-start", type=int, default=2020, help="Start season for analysis")
    parser.add_argument("--season-end", type=int, default=2024, help="End season for analysis")
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    with get_connection() as conn:
        margins, totals = get_game_data(conn, args.season_start, args.season_end)
    ...
    results = generate_teaser_ev_table(
        margins, totals, copula_params, args.output_dir / "teaser_ev_oos_table.tex"
    )

    generate_copula_impact_summary(results, args.output_dir / "teaser_copula_impact_table.tex")
