...
def main():
    ...
    with open(out_dir / "oos_record_table.tex", "w") as f:
        ...
        f.write("  \\centering\n")
        f.write("  \\small\n")
        f.write("  \\caption[Out-of-sample results by season]{Out-of-sample performance by season: GLM baseline, stacked ensemble, and XGBoost (2015--2024).}\n")
        ...
        f.write("    \\toprule\n")
        f.write("    \\textbf{Season} & \\textbf{Model} & \\textbf{N} & \\textbf{Brier $\\downarrow$} & \\textbf{Accuracy} & \\textbf{ROI \\%} \\\\\n")
        f.write("    \\midrule\n")
        ...
        for season in sorted(df_recent['season'].unique()):
            ...
            for idx, (_, row) in enumerate(season_data.iterrows()):
                ...
                # Write row
                if idx == 0:
                    f.write(f"    {season} & {model_short} & {n_games} & {brier} & {acc} & {roi} \\\\\n")
                else:
    ...
    print(f"✅ Generated oos_record_table.tex")
    ...
    for _, row in stack_data.iterrows():
        print(f"  {row['season']}: {row['brier']:.4f} (Acc={row['accuracy']*100:.1f}%, ROI={row['roi']*100:+.1f}%)")
