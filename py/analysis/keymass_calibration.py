...
def generate_calibration_table(
    margins: np.ndarray, counts: np.ndarray, key_numbers: list[int], output_path: Path
):
    ...
    chi2_reweighted, p_reweighted, df_reweighted = chi_square_test(key_observed, key_reweighted)
...
def generate_reweighting_ablation_table(
    ...
    # Reweighted fit
    key_masses = {
        k: observed_probs[np.where(margins == k)[0][0]] if k in margins else 0 for k in key_numbers
    }
    reweighted_probs = apply_ipf_reweighting(base_probs, key_numbers, key_masses)
    ...
    # MAE at key numbers
    mae_base = np.mean(
        [abs(margin_to_count.get(k, 0) - base_probs.get(k, 0) * total) for k in key_numbers]
    )
    mae_rw = np.mean(
        [abs(margin_to_count.get(k, 0) - reweighted_probs.get(k, 0) * total) for k in key_numbers]
    )
...
def main():
    ...
    parser.add_argument("--season-start", type=int, default=1999, help="Start season for analysis")
    ...
    parser.add_argument(
        "--key-numbers", type=int, nargs="+", default=[3, 6, 7, 10, 14], help="Key margins to test"
    )
    ...
    with get_connection() as conn:
        margins, counts = get_margin_distribution(conn, args.season_start, args.season_end)

    print(f"Loaded {len(margins)} unique margins, {counts.sum()} total games")

    # Generate tables
    calibration_stats = generate_calibration_table(
        margins, counts, args.key_numbers, args.output_dir / "keymass_chisq_table.tex"
    )

    ablation_stats = generate_reweighting_ablation_table(
        margins, counts, args.key_numbers, args.output_dir / "reweighting_ablation_table.tex"
    )
