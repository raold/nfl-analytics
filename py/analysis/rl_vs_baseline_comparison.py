...
def kelly_lcb_baseline(conn, season_start=2020, season_end=2024, threshold=0.55, alpha=0.10):
    ...
    if df.empty:
        return {
            'brier': 0.0, 'clv_bps': 0, 'roi_pct': 0.0,
            'max_dd_pct': 0.0, 'sharpe_active': 0.0, 'sharpe_util': 0.0,
            'weeks_active': 0, 'total_bets': 0
        }
    ...
    b = 1.91
    bets['kelly_frac'] = ((bets['p_cover_lcb'] * b - (1 - bets['p_cover_lcb'])) / b).clip(0, 0.05)  # Cap at 5%
    ...
    try:
        ...
        if output_tex:
            ...
            with open(output_tex, 'w') as f:
                ...
                f.write("  \\centering\n")
                f.write("  \\small\n")
                f.write(f"  \\caption{{RL vs stateless baseline ({season_start}–{season_end}, real backtest).}}\n")
            ...
            with open(sharpe_tex, 'w') as f:
                ...
                f.write("  \\centering\n")
                f.write("  \\small\n")
                f.write(f"  \\caption{{Utilization-adjusted Sharpe ({season_start}–{season_end}, real backtest).}}\n")
...
def main():
    ...
    parser.add_argument('--output-tex', type=str, 
