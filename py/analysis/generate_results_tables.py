...
def generate_multimodel_comparison_table():
    ...
    with open(OUT_DIR / "multimodel_comparison_table.tex", 'w') as f:
        ...
        f.write("  \\centering\n")
        f.write("  \\small\n")
        f.write("  \\caption[Multi-Model Backtest Comparison]{Multi-model backtest comparison (2004--2024, $N$=5,529 games). Models ranked by Brier score.}\n")
        ...
        f.write("    \\toprule\n")
        f.write("    \\textbf{Model} & \\textbf{Games} & \\textbf{Brier $\\downarrow$} & \\textbf{LogLoss $\\downarrow$} & \\textbf{Accuracy} & \\textbf{ROI\\%} \\\\\n")
        f.write("    \\midrule\n")
        ...
        for _, row in df.iterrows():
            ...
            f.write(f"    {model_name} & {n_games:,} & {brier} & {logloss} & {acc}\\% & {roi}\\% \\\\\n")

        f.write("    \\bottomrule\n")
        f.write("  \\end{tabular}\n")
        f.write("\\end{table}\n")

    print(f"✅ Generated multimodel_comparison_table.tex")
...
def generate_per_season_summary():
    ...
    with open(OUT_DIR / "per_season_top3_table.tex", 'w') as f:
        ...
        f.write("  \\caption[Per-season performance (top 3 models)]{Per-season performance: GLM baseline, ensemble, and XGBoost (Brier score). Full matrix available in supplementary materials.}\n")
        f.write("  \\label{tab:per-season-top3}\n")
        f.write("  \\setlength{\\tabcolsep}{2.5pt}\\renewcommand{\\arraystretch}{1.08}\n")
        f.write("  \\begin{tabular}{@{} c c c c c c c @{} }\n")
        f.write("    \\toprule\n")
        f.write("    \\textbf{Season} & \\textbf{Model} & \\textbf{N} & \\textbf{Brier} & \\textbf{LogLoss} & \\textbf{Acc \\%} & \\textbf{ROI \\%} \\\\\n")
        f.write("    \\midrule\n")
        ...
        for season in recent_seasons:
            ...
            for _, row in season_df.iterrows():
                model_short = {'glm': 'GLM', 'ens_stack_glm_xgb_state': 'Stack', 'xgb': 'XGB'}[row['model']]
                n = int(row['n_games'])
                brier = format_brier(row['brier'])
                logloss = format_logloss(row['logloss'])
                acc = format_acc(row['accuracy'])
                ...
                f.write(f"    {season} & {model_short} & {n} & {brier} & {logloss} & {acc} & {roi} \\\\\n")
    ...
    print(f"✅ Generated per_season_top3_table.tex")

def generate_rl_comparison_table():
    """Generate RL agent comparison table from JSON."""
    with open(RESULTS_DIR / "rl_agent_comparison.json", 'r') as f:
        data = json.load(f)
    ...
    with open(OUT_DIR / "rl_agent_comparison_table.tex", 'w') as f:
        ...
        f.write("  \\centering\n")
        f.write("  \\small\n")
        f.write("  \\caption[DQN vs PPO Agent Comparison]{DQN vs PPO agent comparison (400 epochs). Q-values and rewards measured on 2020--2024 validation set.}\n")
        f.write("  \\label{tab:rl-comparison}\n")
        f.write("  \\setlength{\\tabcolsep}{4pt}\\renewcommand{\\arraystretch}{1.12}\n")
        f.write("  \\begin{tabular}{@{} l r r r r r @{} }\n")
        f.write("    \\toprule\n")
        f.write("    \\textbf{Agent} & \\textbf{Initial} & \\textbf{Final} & \\textbf{Peak} & \\textbf{Peak Epoch} & \\textbf{Final 50 Std} \\\\\n")
        f.write("    \\midrule\n")

        # DQN row
        dqn = data['dqn']
        f.write(f"    DQN & {dqn['initial_q']:.4f} & {dqn['final_q']:.4f} & {dqn['peak_q']:.4f} & {dqn['peak_epoch']} & {dqn['final_50_std']:.4f} \\\\\n")

        # PPO row
        ppo = data['ppo']
        f.write(f"    PPO & {ppo['initial_reward']:.4f} & {ppo['final_reward']:.4f} & {ppo['peak_reward']:.4f} & {ppo['peak_epoch']} & {ppo['final_50_std']:.4f} \\\\\n")

        f.write("    \\bottomrule\n")
        f.write("  \\end{tabular}\n")
        f.write("\\end{table}\n")

    print(f"✅ Generated rl_agent_comparison_table.tex")
...
def generate_zero_weeks_table():
    ...
    with open(OUT_DIR / "zero_weeks_table.tex", 'w') as f:
        ...
        f.write("  \\centering\n")
        f.write("  \\small\n")
        f.write("  \\caption[Share of zero-bet weeks by season and primary gate]{Share of zero-bet weeks by season and primary gate (requires simulator acceptance logs).}\n")
        f.write("  \\label{tab:zero-weeks}\n")
        f.write("  \\setlength{\\tabcolsep}{4pt}\\renewcommand{\\arraystretch}{1.12}\n")
        f.write("  \\begin{tabular}{@{} c r r r @{} }\n")
        f.write("    \\toprule\n")
        f.write("    \\textbf{Season} & \\textbf{Total Weeks} & \\textbf{Zero-bet (OPE)} & \\textbf{Zero-bet (Sim)} \\\\\n")
        f.write("    \\midrule\n")
    ...
    print(f"✅ Generated zero_weeks_table.tex (placeholder)")
...
def generate_core_ablation_table():
    ...
    with open(OUT_DIR / "core_ablation_table.tex", 'w') as f:
        ...
        f.write("  \\centering\n")
        f.write("  \\small\n")
        f.write("  \\caption[Core ablation grid (mock)]{Core ablation grid: baseline vs RL; reweighting on/off; microstructure features on/off; Gaussian vs $t$-copula. Uses multimodel backtest as proxy for ablation.}\n")
        ...
        f.write("    \\toprule\n")
        f.write("    \\textbf{Config} & \\textbf{Brier $\\downarrow$} & \\textbf{LogLoss $\\downarrow$} & \\textbf{ROI\\%} \\\\\n")
        f.write("    \\midrule\n")
    ...
    print(f"✅ Generated core_ablation_table.tex")
