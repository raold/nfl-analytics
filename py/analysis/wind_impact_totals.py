...
def main():
    ...
    print("-" * 60)
    summary = df.groupby("wind_category").agg({
        "game_id": "count",
        "went_over": ["mean", "sum"],
        "wind_kph": "mean",
        "total_points": "mean",
    ...
    summary.columns = ["n_games", "over_rate", "n_overs", "avg_wind_kph", 
                       "avg_total_points", "avg_total_close", "avg_total_error"]
    print(summary)
    ...
    print(f"Low wind games:  n={len(low_wind)}, over_rate={low_wind['went_over'].mean():.3f}, avg_total={low_wind['total_points'].mean():.1f}")
    print(f"High wind games: n={len(high_wind)}, over_rate={high_wind['went_over'].mean():.3f}, avg_total={high_wind['total_points'].mean():.1f}")
    ...
    print(f"Games with precipitation: n={len(df_precip)}, avg_wind={df_precip['wind_kph'].mean():.1f}, avg_total={df_precip['total_points'].mean():.1f}")
    print(f"Games without precip:     n={len(df_no_precip)}, avg_wind={df_no_precip['wind_kph'].mean():.1f}, avg_total={df_no_precip['total_points'].mean():.1f}")
    ...
    print(f"Under win rate in high wind: {high_wind_under_rate:.3f} ({high_wind_under_wins}/{len(high_wind)})")
