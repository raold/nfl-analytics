...
def main():
    ...
    model_cols = [c for c in df.columns if c not in ['season', 'week', 'game_id', 'home_team', 'away_team', 'home_cover', 'home_score', 'away_score', 'spread_close', 'actual']]
    ...
    print(f"✅ Saved betting_performance_metrics.csv")
    ...
    with open(out_dir / "betting_performance_table.tex", "w") as f:
        ...
        f.write("  \\centering\n")
        f.write("  \\small\n")
        f.write("  \\caption[Betting performance metrics]{Betting performance metrics for top 5 models by Sharpe ratio (2004--2024). Assumes -110 odds, bets placed when model prob $>$ 52.4\\\\%.}\n")
        ...
        f.write("    \\toprule\n")
        f.write("    \\textbf{Model} & \\textbf{N Bets} & \\textbf{Win Rate} & \\textbf{ROI \\%} & \\textbf{Sharpe} & \\textbf{Sortino} \\\\\n")
        f.write("    \\midrule\n")

        for _, row in top5.iterrows():
            model_short = row['model'].replace('ens_stack_', '').replace('ens_mean_', 'MEAN(').replace('_', '+').upper()
            if 'MEAN(' in model_short:
                model_short += ')'

            f.write(f"    {model_short[:25]} & {row['n_bets']:,} & {row['win_rate']*100:.1f}\\% & {row['roi_pct']:+.2f}\\% & {row['sharpe']:.2f} & {row['sortino']:.2f} \\\\\n")

        f.write("    \\bottomrule\n")
        f.write("  \\end{tabular}\n")
        f.write("\\end{table}\n")

    print(f"✅ Generated betting_performance_table.tex")
    ...
    with open(out_dir / "clv_distribution_table.tex", "w") as f:
        ...
        f.write("  \\centering\n")
        f.write("  \\small\n")
        f.write("  \\caption[CLV distribution by model]{Closing line value (CLV) distribution by model (basis points, 2004--2024).}\n")
        ...
        f.write("    \\toprule\n")
        f.write("    \\textbf{Model} & \\textbf{Mean CLV (bps)} & \\textbf{Median CLV (bps)} & \\textbf{Std CLV (bps)} \\\\\n")
        f.write("    \\midrule\n")
        ...
        for _, row in clv_summary.iterrows():
            ...
            f.write(f"    {model_short[:25]} & {row['clv_mean_bps']:+.1f} & {row['clv_median_bps']:+.1f} & {row['clv_std_bps']:.1f} \\\\\n")

        f.write("    \\bottomrule\n")
        f.write("  \\end{tabular}\n")
        f.write("\\end{table}\n")

    print(f"✅ Generated clv_distribution_table.tex")
