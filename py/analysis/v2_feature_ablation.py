...
def load_and_split_data(
    csv_path: Path, test_season: int, val_size: float = 0.2
...
def evaluate_model(model: xgb.Booster, X: np.ndarray, y: np.ndarray, set_name: str) -> dict:
...
def run_ablation_study(
    train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame, device: str = "cuda"
) -> pd.DataFrame:
...
def calculate_incremental_lift(results_df: pd.DataFrame) -> pd.DataFrame:
    ...
    results_df["brier_vs_baseline"] = (
        (baseline_brier - results_df["test_brier"]) / baseline_brier * 100
    )
...
def generate_summary(results_df: pd.DataFrame) -> str:
    ...
    brier_improvement = (
        (baseline["test_brier"] - full_v2["test_brier"]) / baseline["test_brier"] * 100
    )
...
"""
    ...
    if abs(full_v2["brier_vs_baseline"]) > 12:
        recommendation = (
            "**STRONG** - All features contribute meaningfully. Proceed with 13-feature v2 sweep."
        )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("results/ablation"), help="Output directory"
    )
    parser.add_argument(
        "--device", type=str, choices=["cuda", "cpu"], default="cuda", help="Device for training"
    )
    ...
    # Load and split data
    print("\nLoading data...")
    train_df, val_df, test_df = load_and_split_data(args.features_csv, args.test_season)
