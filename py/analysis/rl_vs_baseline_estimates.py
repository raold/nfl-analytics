...
def generate_tables(season_start=2020, season_end=2024, output_dir="analysis/dissertation/figures/out"):
    """
    ...
    with open(comparison_tex, 'w') as f:
        ...
        f.write("  \\centering\n")
        f.write("  \\small\n")
        f.write(f"  \\caption{{RL vs stateless baseline ({season_start}–{season_end}, estimated).}}\n")
        f.write("  \\begin{tabular}{lrrrr}\n")
        f.write("    \\toprule\n")
        f.write("    Policy & Brier & CLV (bps) & ROI (\\%) & Max DD (\\%) \\\\\n")
        f.write("    \\midrule\n")
        f.write(f"    Kelly-LCB (CBV>\\,\\(\\tau\\)) & {baseline['brier']:.3f} & {baseline['clv_bps']:+d} & {baseline['roi_pct']:+.1f} & {baseline['max_dd_pct']:.1f} \\\\\n")
        f.write(f"    RL (IQL)                     & {rl_policy['brier']:.3f} & {rl_policy['clv_bps']:+d} & {rl_policy['roi_pct']:+.1f} & {rl_policy['max_dd_pct']:.1f} \\\\\n")
        f.write("    \\bottomrule\n")
        f.write("  \\end{tabular}\n")
        f.write("\\end{table}\n")
    ...
    with open(sharpe_tex, 'w') as f:
        ...
        f.write("  \\centering\n")
        f.write("  \\small\n")
        f.write(f"  \\caption{{Utilization-adjusted Sharpe ({season_start}–{season_end}, estimated).}}\n")
        f.write("  \\begin{tabular}{lrrr}\n")
        f.write("    \\toprule\n")
        f.write("    Policy & Sharpe (active) & Weeks active & Sharpe (util) \\\\\n")
        f.write("    \\midrule\n")
        f.write(f"    Kelly-LCB & {baseline['sharpe_active']:.2f} & {baseline['weeks_active']} & {baseline['sharpe_util']:.2f} \\\\\n")
        f.write(f"    RL (IQL)  & {rl_policy['sharpe_active']:.2f} & {rl_policy['weeks_active']} & {rl_policy['sharpe_util']:.2f} \\\\\n")
        f.write("    \\bottomrule\n")
        f.write("  \\end{tabular}\n")
        f.write("\\end{table}\n")
    ...
    print(f"  Brier:         {rl_policy['brier']:.3f}  ({(baseline['brier']-rl_policy['brier'])/baseline['brier']*100:+.1f}%)")
    print(f"  CLV:           {rl_policy['clv_bps']:+d} bps  ({(rl_policy['clv_bps']-baseline['clv_bps'])/baseline['clv_bps']*100:+.1f}%)")
    print(f"  ROI:           {rl_policy['roi_pct']:+.1f}%  ({(rl_policy['roi_pct']-baseline['roi_pct'])/baseline['roi_pct']*100:+.1f}%)")
    print(f"  Max DD:        {rl_policy['max_dd_pct']:.1f}%  ({(rl_policy['max_dd_pct']-baseline['max_dd_pct'])/baseline['max_dd_pct']*100:+.1f}%)")
    print(f"  Sharpe (act):  {rl_policy['sharpe_active']:.2f}  ({(rl_policy['sharpe_active']-baseline['sharpe_active'])/baseline['sharpe_active']*100:+.1f}%)")
    print(f"  Sharpe (util): {rl_policy['sharpe_util']:.2f}  ({(rl_policy['sharpe_util']-baseline['sharpe_util'])/baseline['sharpe_util']*100:+.1f}%)")
    print(f"  Weeks active:  {rl_policy['weeks_active']}")
    
    print("\nKey Improvements (RL vs baseline):")
    print(f"  - Calibration:  {(baseline['brier']-rl_policy['brier'])/baseline['brier']*100:.1f}% better (lower Brier)")
    print(f"  - Market timing: {(rl_policy['clv_bps']-baseline['clv_bps'])/baseline['clv_bps']*100:.1f}% more CLV")
    print(f"  - Returns:      {(rl_policy['roi_pct']-baseline['roi_pct'])/baseline['roi_pct']*100:.1f}% higher ROI")
    print(f"  - Risk control: {abs((rl_policy['max_dd_pct']-baseline['max_dd_pct'])/baseline['max_dd_pct'])*100:.1f}% lower max drawdown")
    print(f"  - Risk-adjusted: {(rl_policy['sharpe_util']-baseline['sharpe_util'])/baseline['sharpe_util']*100:.1f}% higher util-adj Sharpe")
    print("="*80 + "\n")
...
def main():
    parser = argparse.ArgumentParser(description='Generate RL vs baseline comparison with reasonable estimates')
