...
class FeatureImportanceAnalyzer:
    ...
    def get_xgboost_importance(self) -> pd.DataFrame:
        ...
        df_pivot = df.pivot(index='feature', columns='importance_type', values='score').reset_index()
    ...
    def analyze_correlation(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        numeric_features = [f for f in self.features if f in df.columns and df[f].dtype in [np.float64, np.int64, np.float32, np.int32]]
    ...
    def recommend_pruning(
        ...
        """
        logger.info(f"Generating pruning recommendations (threshold: {threshold_percentile}th percentile)...")
        ...
        logger.info(f"Recommendation: Keep {recommendations['features_to_keep']}, "
                   f"Prune {recommendations['features_to_prune']} features")
        logger.info(f"Pruned features represent {recommendations['pruned_importance_pct']:.2f}% of total importance")
    ...
    def test_pruned_model(
        ...
        metrics['brier_diff'] = metrics['pruned_model']['brier_score'] - metrics['full_model']['brier_score']
        metrics['accuracy_diff'] = metrics['pruned_model']['accuracy'] - metrics['full_model']['accuracy']

        logger.info(f"Full Model - Brier: {metrics['full_model']['brier_score']:.4f}, "
                   f"Accuracy: {metrics['full_model']['accuracy']:.1%}")
        logger.info(f"Pruned Model - Brier: {metrics['pruned_model']['brier_score']:.4f}, "
                   f"Accuracy: {metrics['pruned_model']['accuracy']:.1%}")
        logger.info(f"Difference - Brier: {metrics['brier_diff']:+.4f}, "
    ...
    def generate_report(
        ...
        report.append(f"- **Pruned Importance**: {recommendations['pruned_importance_pct']:.2f}%\n\n")

        report.append("## Performance Impact\n")
        report.append(f"- **Full Model Brier**: {test_metrics['full_model']['brier_score']:.4f}\n")
        report.append(f"- **Pruned Model Brier**: {test_metrics['pruned_model']['brier_score']:.4f}\n")
        report.append(f"- **Difference**: {test_metrics['brier_diff']:+.4f}\n\n")

        report.append("## Top 20 Features\n")
        for i, row in importance_df.head(20).iterrows():
            report.append(f"{i+1}. **{row['feature']}** - Gain: {row['gain']:.1f}, Weight: {row['weight']:.0f}\n")

        report.append("\n## Bottom 10 Features (Candidates for Pruning)\n")
        for i, row in importance_df.tail(10).iterrows():
            report.append(f"- **{row['feature']}** - Gain: {row['gain']:.1f}, Composite: {row['composite_score']:.4f}\n")
...
def main():
    ...
    parser.add_argument('--test-season', type=int, default=2024,
                       help='Season to use for testing (default: 2024)')
