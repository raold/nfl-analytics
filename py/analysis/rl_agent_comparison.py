...
def compare_training_curves():
    ...
    print(
        f"  Peak Q-value: {max(dqn_q_values):.4f} (epoch {dqn_epochs[dqn_q_values.index(max(dqn_q_values))]})"
    )
    ...
    print(
        f"  Peak reward: {max(ppo_rewards):.4f} (epoch {ppo_epochs[ppo_rewards.index(max(ppo_rewards))]})"
    )
    ...
    print(
        f"  Conclusion: {'PPO more stable' if pd.Series(ppo_final_50).std() < pd.Series(dqn_final_50).std() else 'DQN more stable'}\n"
    )
...
def compare_action_distributions():
    ...
    print(
        f"  Interpreted as: {'Skip/Low' if ppo_avg_action < 0.25 else 'Small' if ppo_avg_action < 0.5 else 'Medium' if ppo_avg_action < 0.75 else 'Large'}"
    )
    ...
    print(
        f"  Conclusion: {'DQN more aggressive' if dqn_bet_rate > ppo_bet_rate else 'PPO more aggressive'}\n"
    )
...
def compare_loss_curves():
    ...
    dqn_spikes = sum(
        1 for l in dqn_losses if l > pd.Series(dqn_losses).mean() + 2 * pd.Series(dqn_losses).std()
    )
    ppo_spikes = sum(
        1 for l in ppo_losses if l > pd.Series(ppo_losses).mean() + 2 * pd.Series(ppo_losses).std()
    )

    print("Loss Spikes (>2σ):")
...
\midrule
Initial Performance & """
        + f"{comparison['dqn']['initial_q']:.4f}"
        + r""" & """
        + f"{comparison['ppo']['initial_reward']:.4f}"
        + r""" \\
Final Performance & """
        + f"{comparison['dqn']['final_q']:.4f}"
        + r""" & """
        + f"{comparison['ppo']['final_reward']:.4f}"
        + r""" \\
Peak Performance & """
        + f"{comparison['dqn']['peak_q']:.4f}"
        + r""" & """
        + f"{comparison['ppo']['peak_reward']:.4f}"
        + r""" \\
Peak Epoch & """
        + f"{comparison['dqn']['peak_epoch']}"
        + r""" & """
        + f"{comparison['ppo']['peak_epoch']}"
        + r""" \\
Training Variance & """
        + f"{comparison['dqn']['variance']:.6f}"
        + r""" & """
        + f"{comparison['ppo']['variance']:.6f}"
        + r""" \\
Final 50 Epoch Std & """
        + f"{comparison['dqn']['final_50_std']:.6f}"
        + r""" & """
        + f"{comparison['ppo']['final_50_std']:.6f}"
        + r""" \\
\midrule
\textbf{Winner} & \multicolumn{2}{c}{\textbf{"""
        + (
            "DQN (lower variance)"
            if comparison["dqn"]["final_50_std"] < comparison["ppo"]["final_50_std"]
            else "PPO (higher reward)"
        )
        + r"""}} \\
\bottomrule
\end{tabular}
