...
def main():
    ...
    with open("TRAINING_COMPLETE_RESULTS.md", "w") as f:
        ...
        for i, r in enumerate(results[:20], 1):
            lr_str = f"{r['lr']}"
            f.write(
                f"| {i} | {r['task_id']} | {r['alpha']:.2f} | {lr_str} | {r['hidden_dims']} | {r['epochs']} | {r['final_loss']:.4f} | {r['q_mean']:.4f} |\n"
            )
        ...
        f.write("|-------|-------------|-------|----------|\n")
        f.write(
            f"| 1 | Quality - 2000 epochs | {len(phase1_2000)} | {min([r['final_loss'] for r in phase1_2000]):.4f} |\n"
        )
        f.write(
            f"| 2 | Exploration - 500 epochs | {len(phase2_500)} | {min([r['final_loss'] for r in phase2_500]):.4f} |\n"
        )
        f.write(
            f"| 3 | Ensemble - 1000 epochs | {len(phase3_1000)} | {min([r['final_loss'] for r in phase3_1000]):.4f} |\n"
        )
        f.write(
            f"| - | Legacy - 200 epochs | {len(old_200)} | {min([r['final_loss'] for r in old_200]):.4f} |\n"
        )
        f.write(
            f"| - | Legacy - 50 epochs | {len(old_50)} | {min([r['final_loss'] for r in old_50]):.4f} |\n"
        )
        ...
        for alpha in sorted(alpha_groups.keys()):
            ...
            f.write(
                f"| {alpha:.2f} | {len(losses)} | {min(losses):.4f} | {sum(losses)/len(losses):.4f} | {max(losses):.4f} |\n"
            )
        ...
        loss_std = (
            sum([(x - loss_mean) ** 2 for x in ensemble_losses]) / len(ensemble_losses)
        ) ** 0.5
        ...
        f.write("|--------|------|------|-------|----------|\n")
        f.write(
            f"| Loss | {min(ensemble_losses):.4f} | {loss_mean:.4f} | {max(ensemble_losses):.4f} | {loss_std:.4f} |\n"
        )
        f.write(
            f"| Q-values | {min(ensemble_q_vals):.4f} | {sum(ensemble_q_vals)/len(ensemble_q_vals):.4f} | {max(ensemble_q_vals):.4f} | - |\n"
        )
        ...
        f.write(
            f"4. **Training duration**: 2000 epochs achieved {best['final_loss']:.4f} loss (vs ~0.09-0.14 @ 200 epochs)\n"
        )
        f.write(
            f"5. **Improvement**: {((0.09-best['final_loss'])/0.09*100):.1f}% better than initial short training\n"
        )
        f.write(
            f"6. **Ensemble ready**: 20 models with mean loss {loss_mean:.4f} ± {loss_std:.4f}\n"
        )
    ...
    print("✅ Results saved to TRAINING_COMPLETE_RESULTS.md and all_cql_results.csv")
