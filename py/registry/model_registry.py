...
class ModelVersion:
    ...
    parent_version: Optional[str] = None  # Parent version for lineage
...
class ModelRegistry:
    ...
    def promote_model(
        ...
        if new_status not in valid_transitions.get(model.status, []):
            raise ValueError(
                f"Cannot promote from {model.status} to {new_status}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="NFL Analytics Model Registry"
    )
    parser.add_argument(
        "command",
    ...
    if args.command == "register":
        ...
        comparison = registry.compare_versions(
            args.name, args.version, args.version2
        )
