...
def calculate_prop_ev(model_prob: float, odds: int, bet_amount: float = 100.0) -> float:
    """
...
def calculate_prop_correlation(
    player_id: str, prop_type1: str, prop_type2: str, historical_data: pd.DataFrame
) -> float:
    """
...
class PropsEVSelector:
    ...
    def select_props(
        ...
        merged = predictions_df.merge(prop_lines_df, on=["player_id", "prop_type"], how="inner")
...
def main():
    parser = argparse.ArgumentParser(description="EV-Optimal Prop Bet Selector")
    ...
    parser.add_argument(
        "--deadzone", type=float, default=0.005, help="No-bet band (default: 0.005)"
    )
    parser.add_argument(
        "--max-hold", type=float, default=0.10, help="Maximum book hold (default: 0.10)"
    )
    parser.add_argument(
        "--min-ev", type=float, default=3.0, help="Minimum EV per $100 (default: 3.0)"
    )

    # Kelly sizing
    parser.add_argument(
        "--kelly-fraction", type=float, default=0.15, help="Kelly fraction (default: 0.15)"
    )
    parser.add_argument(
        "--max-bet-pct", type=float, default=0.03, help="Max bet pct of bankroll (default: 0.03)"
    )
    parser.add_argument(
    ...
    # Flags
    parser.add_argument(
        "--no-injury-check", action="store_true", help="Skip injury status checking"
    )
    ...
    print("\nTop 10 bets by EV:")
    print(
        selected.nlargest(10, "ev")[
            ["player_name", "prop_type", "bet_side", "line_value", "edge", "ev", "bet_amount"]
        ]
    )
