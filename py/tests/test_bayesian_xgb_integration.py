...
def load_bayesian_predictions(conn, season: int = 2024, week: int = 7, stat_type: str = 'passing_yards') -> pd.DataFrame:
...
def test_inverse_variance_weighting():
    ...
    assert abs(combined_mean - expected_mean) < 0.01, f"Expected {expected_mean}, got {combined_mean}"
    logger.info(f"✓ Equal uncertainty test passed: {combined_mean:.2f} (expected {expected_mean:.2f})")
    ...
    assert abs(combined_mean - bayesian_mean) < abs(combined_mean - xgb_mean), \
        "Combined mean should be closer to more confident prediction"
    logger.info(f"✓ Weighted average test passed: {combined_mean:.2f} (closer to {bayesian_mean} than {xgb_mean})")

    # Combined uncertainty should be lower than both individual uncertainties
    assert combined_std < min(bayesian_std, xgb_std), \
        f"Combined std ({combined_std:.2f}) should be less than min individual std ({min(bayesian_std, xgb_std):.2f})"
    logger.info(f"✓ Combined uncertainty test passed: {combined_std:.2f} < {min(bayesian_std, xgb_std):.2f}")
...
def test_integration_with_mock_data():
    ...
    logger.info(f"Sample prior: {bayesian_priors.iloc[0]['predicted_value']:.1f} ± {bayesian_priors.iloc[0]['predicted_std']:.1f}")
    ...
    with psycopg.connect(**DB_CONFIG) as conn:
        ...
        for stat_type in ['passing_yards', 'rushing_yards', 'receiving_yards']:
            ...
            if not df.empty:
                ...
                yards_q05 = np.exp(sample['rating_q05']) if sample['rating_q05'] else yards_mean * 0.7
                yards_q95 = np.exp(sample['rating_q95']) if sample['rating_q95'] else yards_mean * 1.3
